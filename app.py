#Creación Rápida de Archivos de Script fuera del notebook)
...
if st.button("Predecir ingreso"):
    ...
    # C. Probabilidades por categoría (con el monto estimado como referencia sobre el gráfico)
    ...
    barras = ax.bar(proba_df['Categoría'], proba_df['Probabilidad'],
                     color=[COLOR_CATEGORIA[c] for c in proba_df['Categoría']])
    ax.set_ylim(0, 1)
    ax.set_ylabel("Probabilidad")
    ax.set_title(f"Ingreso estimado: {ingreso_predicho:,.1f}", fontsize=11, fontweight='bold')

    # Se resalta con una etiqueta el monto estimado sobre la barra de la categoría predicha
    for barra, cat in zip(barras, proba_df['Categoría']):
        if cat == categoria:
            ax.annotate(f"{ingreso_predicho:,.1f}",
    ...
    # D. Ficha de validación (incluye el monto de ingreso estimado)
    st.markdown("### Ficha de validación")
    etiquetas = ['Edad', 'Años en dirección actual', 'Gasto en coche', 'Años de empleo',
                 'Años de residencia', 'Ingreso estimado']
    valores = [edad, anios_direccion, gasto_coche, anios_empleo, anios_residen,
               round(float(ingreso_predicho), 1)]
